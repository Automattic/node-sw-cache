...
CertUtil.registerSSL = function( hostname, ssl ) {
	...
	if ( os.platform() === 'darwin' ) {
		console.log( [
			'Either enter your password to add the certificate to your System or run the following command and restart:',
			'\tsudo security add-trusted-cert -d -r trustRoot -k /Library/Keychains/System.keychain ' + certPath
		].join( '\n' ) );
		sudo( ( 'security add-trusted-cert -d -r trustRoot -k /Library/Keychains/System.keychain ' + certPath ).split( ' ' ), {
			cachePassword: false,
			prompt: '[sudo] password for ' + process.env.USER + ': '
		} );
	} else {
